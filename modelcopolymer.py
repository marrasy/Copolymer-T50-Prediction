...
from torch_geometric.nn import CGConv, GlobalAttention, NNConv, Set2Set, AttentionalAggregation
...
class SingleTaskGATNet(torch.nn.Module):
    def __init__(self, node_in_len, edge_in_len, fea_len, n_layers, n_h, heads=2):
        super(SingleTaskGATNet, self).__init__()
        self.node_embed = Linear(node_in_len, fea_len)
        self.edge_embed = Linear(edge_in_len, fea_len)

        self.gatconvs = nn.ModuleList([
            GATConv(fea_len, fea_len // heads, heads=heads)
            for _ in range(n_layers)])
    ...
    def forward(self, data):
        ...
        for gatconv in self.gatconvs:
            out = gatconv(out, data.edge_index)
